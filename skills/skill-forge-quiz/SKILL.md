...
### 数学公式（LaTeX 规范）
当题目/解析里包含数学表达式（例如 `f(x)=e^{3x}`、极限、积分、矩阵等）时，**必须使用 LaTeX 语法**：
- **行内公式**：使用 `$...$`，例如：`如果 $f(x)=e^{3x}$，求 $f'(x)$。`
- **独立公式**：使用 `$$...$$`（适合较长推导或多行公式）

常见写法示例：
- 极限：`$\\lim_{x\\to a} f(x) = L$`（不要写成 `lim_{x->a}` 这种纯文本）

注意事项：
- 生成 JSON 时，LaTeX 依然是普通字符串；必要时对反斜杠进行转义（例如 `\\frac{a}{b}`）。
- **只有被分隔符包裹的 LaTeX 才会被渲染**：请确保公式在 `$...$` 或 `$$...$$` 内，否则页面会按普通文本显示。
- 不要把纯文本“公式描述”混在一起导致歧义，公式部分尽量用 LaTeX 包裹。
